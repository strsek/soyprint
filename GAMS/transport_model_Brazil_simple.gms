...
Parameter ctransport(a,b) table of pair-wise transport cost for each MU pair in $ per ton
$call CSV2GDX  .\MUN_dist_long.csv output = .\transport_cost.gdx id=ctransport index = 1,2 values = 3 useHeader= y trace = 3
...
Parameter
*modal_split(mode) 'modal split (share of each mode on total soy transport kilometers in Brazil)' 
...
Positive variable
    xsoytransport(a,b,product) 'transport from a to b in tons'
;
...
cost.. 
    SUM((a,b,product), ctransport(a,b) *  xsoytransport(a,b,product)$(supply(a,product) and demand(b,product))) =E= xtotalcost  ;

demand_const(b, product)..
    SUM((a), xsoytransport(a,b,product)$(supply(a,product) and demand(b,product))) =G= demand(b,product);

supply_const(a, product)..
    SUM((b), xsoytransport(a,b,product)$(supply(a,product) and demand(b,product))) =L= supply(a,product) ;
  
*totaltransport(product)..
*    SUM((a,b), xsoytransport(a,b,product)$(supply(a,product) and demand(b,product))) =E= xtotalsoytransport(product);

* impose modal split on overall tranport, not differentiating between products  
*modal_const(mode)..
*    SUM((a,b,product), xsoytransport(a,b,product)$(supply(a,product) and demand(b,product))) =E= SUM(product, xtotalsoytransport(product) * modal_split(mode));

* impose modal split seperately on each product
*modal_const(mode, product)..
*    SUM((a,b), xsoytransport(a,b,product)$(supply(a,product) and demand(b,product))) =E= xtotalsoytransport(product) * modal_split(mode);
...
report1(product, "total cost") = SUM((a,b), ctransport(a,b) *  xsoytransport.l(a,b,product));
...
report2(product,a,b) =  SUM(mode, xsoytransport.l(a,b,product))
display report2

$offtext


execute_unload 'transport_sol_simple.gdx',xsoytransport,xtotalcost;
*xtotalsoytransport
