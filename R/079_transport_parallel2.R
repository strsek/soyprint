...
# distance matrices
D_a_b <- list(name='D_a_b',val=road_dist_MUN, uels=dimnames(road_dist_MUN),
              dim=2, domains = c("a", "b"), form='full',type='parameter',
              ts='road transportation distance between all MUs')
D_a_w1 <- list(name='D_a_w1',val=road_dist_MUN_port, uels=dimnames(road_dist_MUN_port),
               dim=2, domains = c("a", "w1"), form='full',type='parameter',
               ts='road transportation distance between MUs and origin ports')
D_a_r1  <- list(name='D_a_r1',val=road_dist_MUN_stat, uels=dimnames(road_dist_MUN_stat),
                dim=2, domains = c("a", "r1"), form='full',type='parameter',
                ts='road transportation distance between MUs and origin stations')
D_w2_b <- list(name='D_w2_b',val=road_dist_port_MUN, uels=dimnames(road_dist_port_MUN),
               dim=2, domains = c("w2", "b"), form='full',type='parameter',
               ts='road transportation distance between MUs and origin ports')
D_r2_b <- list(name='D_r2_b',val=road_dist_stat_MUN, uels=dimnames(road_dist_stat_MUN),
               dim=2, domains = c("r2", "b"), form='full',type='parameter',
               ts='road transportation distance between MUs and origin ports')
D_r1_r2 <- list(name='D_r1_r2',val=rail_dist, uels=dimnames(rail_dist),
                dim=2, domains = c("r1", "r2"), form='full',type='parameter',
                ts='rail transportation distance between rail terminals')
D_w1_w2 <- list(name='D_w1_w2',val=water_dist, uels=dimnames(water_dist),
                dim=2, domains = c("w1", "w2"), form='full',type='parameter',
                ts='water transportation distance between ports')


wgdx.lst("GAMS/GAMS_base_data2.gdx", 
           list(product_lst, a_lst, 
                w1_list, w2_list, r1_list, r2_list, 
                supply, demand, exp_proc, 
                cap_w, cap_r,
                D_a_b, D_a_w1, D_a_r1, D_w2_b, D_r2_b, D_r1_r2, D_w1_w2))
...
tibble(id = integer(), c_road = double(), c_rail_short = double(), c_rail_long = double(), c_water = double(), m_switch = double()) %>% 
...
system.time(
  ...
  foreach(
    ...
    gdx_cost2 <- tempfile(tmpdir = "./GAMS/bs_tmp", fileext = ".gdx")
    gdx_out2 <- tempfile(tmpdir = "./GAMS/bs_tmp", fileext = ".gdx")
    ...
    C_road <- list(name='C_road',val=c_road, 
                  #type='scalar',
                  ts='road transportation costs per tkm')
    C_rail_short <- list(name='C_rail_short',val=c_rail_short, 
                   #type='scalar',
                   ts='rail transportation costs per tkm short')
    C_rail_long  <- list(name='c_rail_long',val=c_rail_long, 
                    ts='rail transportation costs per tkm long')
    C_water <- list(name='C_water',val=c_water, 
                   #type='scalar',
                   ts='water transportation costs per tkm')
    M_switch <- list(name='M_switch',val=m_switch, 
                   #type='scalar',
                   ts='switching costs per t')


    # write
   wgdx.lst(gdx_cost2, 
            list(C_road, C_rail_short, C_rail_long, C_water, M_switch))
   

   
    # append parameter information to par file
    tibble(id = i, c_road, c_rail_short, c_rail_long, c_water, m_switch) %>% 
    ...
    totalcost <- rgdx(outgdx, requestList = list(name = "xtotalcost"))$val
