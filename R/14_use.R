...
# what is changed:
# - use table is extended for 
# -- municipal soy use soy processes themselves (p021 as seedwaste, p067 as input for crushing) 
# -- municipal feed use of soy in livestock sectors, using own estimates from 03_feed_use.R (NOTE that only soy feed is known on municipal level, all other feed uses remain on the national level)
...
cbs <- readRDS("intermediate_data/FABIO/cbs_full.rds") 
sup <- readRDS("intermediate_data/FABIO/sup.rds")

use_items <- fread("input_data/FABIO/inst/items_use.csv")
...
cat("Allocating crops going directly to a process. Applies to items:\n\t",
  ...
  ".\n", sep = "")
use[type == "100%", `:=`(use = processing, processing = 0)] # why is processing set to 0 here?
...
cbs[!is.na(use), processing := na_sum(processing, -use)] # why?
cbs[, use := NULL]
...
tcf_cbs <- fread("input_data/FABIO/inst/tcf_cbs.csv")
...
cbs <- merge(cbs, results[, list(value = na_sum(value)),
  ...
  by = c("area_code", "year", "item_code"), all.x = TRUE)
cbs[!is.na(value), processing := na_sum(processing, -value)] #round(na_sum(processing, -value))
cbs[, value := NULL]
...
eth_tcf <- fread("input_data/FABIO/inst/tcf_eth.csv")
...
# Use animal stocks
live <- readRDS("input_data/FABIO/tidy/live_tidy.rds")
...
conv_k <- fread("input_data/FABIO/inst/conv_krausmann.csv")
...
conv_b <- fread("input_data/FABIO/inst/conv_bouwman.csv")
conc_b <- fread("input_data/FABIO/inst/conc_bouwman.csv")
...
live_b <- live[element == "Production" & unit == "tonnes" & year %in% unique(use$year), ] ### added year
...
feed_req_b[, converted := production * conversion] #round(production * conversion, 3)
...
feed_req_b[, `:=`(residues = residues - fodder, item = NULL)] # residues = round(residues - fodder, 3)
...
bean_feed_mun <- readRDS("intermediate_data/bean_feed_t.rds")
cake_feed_mun <- readRDS("intermediate_data/cake_feed_t.rds")
...
#saveRDS(feed_mun, "intermediate_data/FABIO/feed_mun.RDS") # just for sanity checks
...
use[, comm_code := items$comm_code[conc]] ## is that redundant?
...
opt_tcf <- fread("input_data/FABIO/inst/tcf_optim.csv")
opt_in <- fread("input_data/FABIO/inst/optim_in.csv")
opt_out <- fread("input_data/FABIO/inst/optim_out.csv")
...
# TODO: add excess feed supply to unspecified
cbs <- merge(cbs, feed_excess, by = c("year","item_code","item","area_code","area"), all.x = TRUE)
cbs[, `:=`(unspecified = na_sum(unspecified, sup_excess))]
cbs[, sup_excess := NULL]
setkey(cbs, year, area_code, comm_code)
...
setkey(use, year, area_code, comm_code, proc_code, type)
setkey(use_fd, year, area_code, comm_code)


# Save -----

if (write){
  saveRDS(cbs, "intermediate_data/FABIO/cbs_final.rds")
  saveRDS(use, "intermediate_data/FABIO/use_final.rds")
