...
trase_mun <- trase_mun %>% 
  left_join(dplyr::select(regions, CO_PAIS_ISOA3, ISO_BTD), 
            by = c("co_dest" = "CO_PAIS_ISOA3")) %>%
...
trase_mun <- trase_mun %>% 
  rename(co_mun = co_mun_trase) #%>% left_join(select(SOY_MUN, c(co_mun, co_state)), by = "co_mun") %>% relocate(co_state, .before = nm_state)
...
regions_btd <- filter(regions, ISO_BTD != "ROW", btd == TRUE) %>% 
  distinct(CO_BTD, ISO_BTD, region)
comp_mun <- comp_mun %>% 
  left_join(dplyr::select(regions_btd, c(ISO_BTD, region)), by = c("to_code" = "ISO_BTD")) %>%
  rename(to_region = region) %>% 
  relocate(to_region, .after = to_code) %>%
  # separate China
...
(diff_mun <- comp_mun %>% 
    filter(co_mun != 9999999) %>%  
    dplyr::select(contains("diff")) %>% 
    abs() %>% 
    colSums())
(diff_state <- comp_state %>% 
    filter(!is.na(co_state)) %>% 
    dplyr::select(contains("diff")) %>% 
    abs() %>% 
    colSums())
(diff_state_by_region <- comp_state_by_region %>% 
    filter(!is.na(co_state)) %>% 
    dplyr::select(contains("diff")) %>% 
    abs() %>% 
    colSums())
...
#
#
#ggplot(comp_mun, aes(x=trase, y = intermod), color = "blue")+
