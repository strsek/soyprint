...
mu_to_mu <- mclapply(names(flows), function(nm){
  ...
  ## WE don't even need the long mapping! We can create full sparse matrix directly!
...
#names(mu_to_mu_df) <- c("co_orig", "co_dest", "product", names(mu_to_mu))
...
#mu_to_mu_df$euclid <- as.numeric(mu_to_mu_df$euclid)
