...
SOY_MUN_cbs <- SOY_MUN %>% dplyr::select(prod_bean, prod_oil, prod_cake, imp_bean, imp_oil, imp_cake, exp_bean, exp_oil, exp_cake, food_bean, food_oil, feed_bean, feed_cake, seed_bean, other_oil, proc_bean, stock_bean:stock_cake)
...
SOY_MUN  <- SOY_MUN %>%  mutate(total_supply_bean = prod_bean + imp_bean, 
...
SOY_MUN <- dplyr::select(SOY_MUN, -c(proc_fac:storage_cap, exp_bean_d, exp_oil_d, exp_cake_d, imp_bean_d, imp_oil_d, imp_cake_d ))

# merge with GEO file
GEO_MUN_SOY <- right_join(GEO_MUN_SOY, dplyr::select(SOY_MUN, c(co_mun, total_supply_bean:excess_use_cake)), by = "co_mun")
