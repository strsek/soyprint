...
# edit: further remove livestock processes that actually are not using soy according to our MU estimtes
processes_mun_use <- processes_mun_use[!processes_mun_use %in% c("p087", "p088", "p091", "p092", "p093", "p094", "p095", "p096", "p097", "p101", "p102", "p103")]
...
# TODO: adapt to new structure --> done --> verify!
...
mr_sup_mass <- lapply(years, function(x) {
  ...
  matrices <- lapply(areas, function(y, sup_y) {
    ...
    ## TODO: adaption for MUs: use only soy item commodities and processes --> done --> verify!
    ...
    ## TODO: adaption for Brazil:remove soy item commodities and processes --> done --> verify!
    ...
    ## TODO: add area code as prefix to names --> done --> verify!
    return(Matrix(data.matrix(out[, c(-1)]), sparse = TRUE,
  ...
  # TODO: check that bdiag item has named rows/columns
...
## modify to contain soy trade on MUN basis:
...
rm(btd, template )
saveRDS(btd_cast, "intermediate_data/FABIO/btd_cast.rds")
...
## TODO: the comms for soy appear 188 + n(MUN) times, while all others appear 189 times  --> done --> verify!
...
supply_shares <- lapply(btd_cast, function(x, agg, js) {
  ...
  ## TODO: adapt repeat of denom  --> done --> verify!
  ...
  # TODO: check what this means --> for grazing!
  # --> adapt by using named indexing!
...
saveRDS(supply_shares, "intermediate_data/FABIO/supply_shares.rds")
rm(agg, btd_cast, cbs)
gc()


# TODO: check why some colSums of supply_shares are not integers --> corrected

# Use ---

# Template to always get full tables
# TODO: reduce to soy processes for MUs
# TODO: make sure the correct processes are included for brazil and MUs
template <- data.table(expand.grid(
  area_code = areas_w, proc_code = processes[!processes %in% processes_soy], comm_code = commodities,
...
template_soy <- data.table(expand.grid(
  area_code = areas_soy, proc_code = processes_mun_use, comm_code = commodities,
  stringsAsFactors = FALSE))
template <- rbind(template, template_soy)
...
saveRDS(use_cast, "intermediate_data/FABIO/use_cast.RDS")
rm(use, template)
...
mr_use <- mapply(function(x, y) {
  ...
  ## TODO: adapt to new structure --> done --> verify!
  # we make sure that the origin of commodities in the rows conforms with the supply mr_sup matrix
  ...
  ## The for loop is inefficient for >5000 areas!!!
  #for(j in seq_along(areas)) { # Per country j
  #  mr_x[, seq(1 + (j - 1) * n_proc, j * n_proc)] <-
  #    mr_x[, seq(1 + (j - 1) * n_proc, j * n_proc)] * y[, j]
  #}
  
  #for(j in (areas)) { # Per country j
  #  mr_x[, grepl(paste0(j,"_"), colnames(mr_x))] <-
  #    mr_x[, grepl(paste0(j,"_"), colnames(mr_x))] * y[, as.character(j)]
  #}
...
## TODO: aggregate municipalities to national processes where detail is not needed
# done --> verify!
...
saveRDS(mr_use_fd, "intermediate_data/FABIO/Y.rds")
