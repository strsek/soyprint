...
btd_imp <- readRDS("input_data/FABIO/btd_bal.rds") %>% filter(year == 2013, item_code %in% soy_items)
btd_exp <- readRDS("input_data/FABIO/FABIO_exp/btd_bal.rds") %>% filter(year == 2013, item_code %in% soy_items)
btd_exp_0 <- readRDS("input_data/FABIO/FABIO_exp/v0/btd_bal.rds") %>% filter(year == 2013, item_code %in% soy_items)
btd_exp_1 <- readRDS("input_data/FABIO/FABIO_exp/v1/btd_bal.rds") %>% filter(year == 2013, item_code %in% soy_items)
#btd_final <- readRDS("input_data/FABIO/FABIO_exp/btd_final.rds")
cbs <- readRDS("input_data/FABIO/cbs_full.rds")
#FAO trade matrix
trade_mat <- read.csv("input_data/FABIO/FAOSTAT_tradematrix_BRAsoy.csv")
...
regions <- regions %>% mutate(ISO_BTD = ifelse(CO_PAIS_ISOA3 %in% c("GLP", "GUF", "MTQ", "REU"), "FRA", ISO_BTD), CO_BTD = ifelse(CO_PAIS_ISOA3 %in% c("GLP", "GUF", "MTQ", "REU"), 68, CO_BTD))
...
btd_exp_BRA_soy <- filter(btd_exp, from_code == 21, item_code %in% c(2555, 2571, 2590))
btd_imp_BRA_soy <- filter(btd_exp, to_code == 21, item_code %in% c(2555, 2571, 2590))

btd_exp_BRA_soy0 <- filter(btd_exp_0, from_code == 21, item_code %in% c(2555, 2571, 2590))
btd_imp_BRA_soy0 <- filter(btd_exp_0, to_code == 21, item_code %in% c(2555, 2571, 2590))

btd_exp_BRA_soy1 <- filter(btd_exp_1, from_code == 21, item_code %in% c(2555, 2571, 2590))
btd_imp_BRA_soy1 <- filter(btd_exp_1, to_code == 21, item_code %in% c(2555, 2571, 2590))
...
rm(btd_imp,btd_exp,btd_exp_0,btd_exp_1 )
...
EXP_NAT_SOY <- EXP_NAT_SOY %>% full_join(btd_exp_BRA_soy[,c(2,4:5)], by = c("item_code" = "item_code", "to_code" = "to_code")) %>% rename("export_btd_exp" = "value")
EXP_NAT_SOY <- EXP_NAT_SOY %>% full_join(btd_exp_BRA_soy_imp[,c(2,4:5)], by = c("item_code" = "item_code", "to_code" = "to_code")) %>% rename("export_btd_imp" = "value")
EXP_NAT_SOY <- EXP_NAT_SOY %>% full_join(btd_exp_BRA_soy0[,c(2,4:5)], by = c("item_code" = "item_code", "to_code" = "to_code")) %>% rename("export_btd_exp0" = "value")
EXP_NAT_SOY <- EXP_NAT_SOY %>% full_join(btd_exp_BRA_soy1[,c(2,4:5)], by = c("item_code" = "item_code", "to_code" = "to_code")) %>% rename("export_btd_exp1" = "value")

IMP_NAT_SOY <- IMP_NAT_SOY %>% full_join(btd_imp_BRA_soy[,c(2:3,5)], by = c("item_code" = "item_code", "from_code" = "from_code")) %>% rename("import_btd_exp" = "value")
IMP_NAT_SOY <- IMP_NAT_SOY %>% full_join(btd_imp_BRA_soy_imp[,c(2:3,5)], by = c("item_code" = "item_code", "from_code" = "from_code")) %>% rename("import_btd_imp" = "value")
IMP_NAT_SOY <- IMP_NAT_SOY %>% full_join(btd_imp_BRA_soy0[,c(2:3,5)], by = c("item_code" = "item_code", "from_code" = "from_code")) %>% rename("import_btd_exp0" = "value")
IMP_NAT_SOY <- IMP_NAT_SOY %>% full_join(btd_imp_BRA_soy1[,c(2:3,5)], by = c("item_code" = "item_code", "from_code" = "from_code")) %>% rename("import_btd_exp1" = "value")
...
trade_mat <- mutate(trade_mat, product = ifelse(Item.Code == 236, "soybean", ifelse(Item.Code == 237, "soy_oil", ifelse(Item.Code == 238, "soy_cake", "other")))) %>% filter(product != "other")
trade_mat <- trade_mat %>% rename("to_name" = "Partner.Country.Code..ISO3.", "FAO_trade_mat" = "Value") %>% dplyr::select(c(to_name, product, FAO_trade_mat)) %>% mutate(to_name = ifelse(to_name == "41", "CHN", to_name))
trade_mat <- trade_mat %>% left_join(distinct(dplyr::select(regions,c(ISO_BTD, CO_PAIS_ISOA3)) %>% rename(to_name = CO_PAIS_ISOA3)), by = ("to_name"))
trade_mat_agg <- group_by(trade_mat, ISO_BTD, product) %>% summarise(FAO_trade_mat = sum(FAO_trade_mat, na.rm = TRUE), .groups = "drop") %>% rename(to_name = ISO_BTD)
EXP_NAT_SOY <- full_join(EXP_NAT_SOY, trade_mat_agg, by = c("product", "to_name"))

# replace NAs
EXP_NAT_SOY <-  EXP_NAT_SOY %>% replace(is.na(.), 0)
IMP_NAT_SOY <-  IMP_NAT_SOY %>% replace(is.na(.), 0)
...
colSums(EXP_MUN_SOY[,c(10)], na.rm = TRUE)
colSums(IMP_NAT_SOY[,c(5,7)], na.rm = TRUE)
colSums(IMP_MUN_SOY[,c(10)], na.rm = TRUE)
...
#EXP_SOY <- EXP_NAT_SOY %>% group_by(product) %>% summarise(export = sum(export, na.rm = T),  export_btd_exp = sum(export_btd_exp, na.rm = T), export_btd_imp = sum(export_btd_imp, na.rm = T))
EXP_SOY <- EXP_NAT_SOY %>% group_by(product) %>% summarise(across(starts_with('export'), .fns = sum, na.rm = TRUE))
...
#IMP_SOY <- IMP_NAT_SOY %>% group_by(product) %>% summarise(import = sum(import, na.rm = T), import_dol = sum(import_dol, na.rm = T), import_btd_exp = sum(import_btd_exp, na.rm = T), import_btd_imp = sum(import_btd_imp, na.rm = T))
IMP_SOY <- IMP_NAT_SOY %>% group_by(product) %>% summarise(across(starts_with('import'), .fns = sum, na.rm = TRUE))
