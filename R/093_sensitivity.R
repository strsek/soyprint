...
#sensi_mu <- scatter_mu + dens_mu +
#  plot_layout(ncol = 2, nrow = 1, widths = c(4, 1), heights = c(4))


(sensi_mu <- dens_mean + plot_spacer() + scatter_mu + dens_cv + 
  plot_layout(ncol = 2, nrow = 2, widths = c(4, 0.8), heights = c(1, 4)))

if (write) ggsave(filename = "results/sensitivity/sensi_mu.png", height = 5, width = 7, units = "in", sensi_mu, bg = "white")
...
if (write) ggsave(filename = "results/sensitivity/sensi_cum_mu.png", height = 5, width = 6, units = "in", cumplot_mu, bg = "white")
...
if (write) ggsave(filename = "results/sensitivity/sensi_trase.png", sensi_trase, height = 5, width = 7, units = "in", bg = "white")

## Create a structure to map importers to exporters per item (+ targets)
#mapping_templ <- as.data.table(expand.grid(co_orig = co_mun, co_dest = co_mun, product = product))

#flows_comp <- mclapply(names(flows), function(x){
#
#  flow_long <- merge(mapping_templ, flows[[x]], by = c("co_orig", "co_dest", "product"), all.x = TRUE)  %>% 
#    replace_na(list(value = 0))
#  
#  setnames(flow_long, "value",  str_c(x))
#  return(flow_long)
#},  mc.cores = 12) %>%   
#  reduce(merge, by = c("co_orig", "co_dest", "product"), all=TRUE) 
...
if (write) ggsave(filename = "results/sensitivity/sensi_cum_trase.png", height = 5, width = 6, units = "in", cumplot_trase, bg = "white")
